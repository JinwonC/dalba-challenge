...
// 기존 리뷰 행 중 메타(크리에이터/핸들/링크/제목/게시일)가 빈 것을 원본에서 id로 채움. (1회성)
export async function backfillReviewMeta() {
  const { rows } = await readReviewRows();
  const { header, rows: vrows } = await readVideoTable();
  const H = {}; header.forEach((h, i) => { H[String(h).trim()] = i; });
  const gg = (row, name) => (H[name] != null ? row[H[name]] : '');
  const vmap = {};
  for (const vr of vrows) {
    const id = String(gg(vr, 'id') || '').replace(/^'/, '').trim();
    if (!id || vmap[id]) continue;
    const handle = gg(vr, 'username') || '';
    vmap[id] = {
      postDate: normDate(gg(vr, 'video_post_time')),
      creator: gg(vr, 'creator.nick_name') || '',
      handle,
      link: gg(vr, 'Video Link') || (handle ? `https://www.tiktok.com/@${handle}/video/${id}` : ''),
      title: gg(vr, 'title') || '',
    };
  }
  let filled = 0, matched = 0;
  const out = rows.map((r) => {
    const row = r.slice(); while (row.length < RC) row.push('');
    const id = String(row[0] || '').replace(/^'/, '').trim();
    const m = vmap[id];
    if (m) {
      matched++;
      let did = false;
      if (!row[1] && m.postDate) { row[1] = m.postDate; did = true; }
      if (!row[2] && m.creator) { row[2] = m.creator; did = true; }
      if (!row[3] && m.handle) { row[3] = m.handle; did = true; }
      if (!row[4] && m.link) { row[4] = m.link; did = true; }
      if (!row[5] && m.title) { row[5] = m.title; did = true; }
      if (did) filled++;
    }
    return row;
  });
  if (out.length) await updateRange(`${q(REVIEW_TAB)}!A2:${LAST_COL}${out.length + 1}`, out);
  return { totalReviews: out.length, matchedInSource: matched, rowsFilled: filled };
}

// 리뷰 탭 원본 덤프(디버그/검증용)
export async function dumpReviewTab() {
  try {
    return (await api(`/values/${enc(q(REVIEW_TAB))}!A:${LAST_COL}`)).values || [];
  } catch (e) {
    if (/Unable to parse range/i.test(String(e.message || ''))) return [];
    throw e;
  }
