...
const TAB_NAME = process.env.SHEET_TAB || '영상성과_신API테스트';
// 한글/특수문자 탭명은 A1 range에서 반드시 작은따옴표로 감싸야 함 ('' 로 이스케이프)
const TAB = "'" + TAB_NAME.replace(/'/g, "''") + "'";
...
export async function listTabsOf(sid) {
  const tk = await token();
  const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sid}?fields=${enc('sheets.properties.title')}`, {
...
export async function readHeadOf(tabName) {
  const q = "'" + String(tabName).replace(/'/g, "''") + "'";
  const j = await api(`/values/${enc(q + '!1:1')}`);
  return (j.values && j.values[0]) || [];
}
export async function readRangeOf(tabName, a1) {
  const q = "'" + String(tabName).replace(/'/g, "''") + "'";
  const j = await api(`/values/${enc(q + '!' + a1)}`);
  return j.values || [];
}
export async function listTabs() {
  const j = await api(`?fields=${enc('sheets.properties.title')}`);
  return (j.sheets || []).map((s) => s.properties && s.properties.title);
}
...
export async function readColumn(letter) {
...
export async function ensureReviewCols(header) {
