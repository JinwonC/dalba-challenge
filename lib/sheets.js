...
async function ensureReviewTab() {
  const titles = await listTabTitles();
  if (!titles.includes(REVIEW_TAB)) {
...
// 리뷰 탭 전체를 { id: {rating, note} } 맵으로
export async function readReviews() {
  const titles = await listTabTitles();
  if (!titles.includes(REVIEW_TAB)) return {};
  const j = await api(`/values/${enc(q(REVIEW_TAB))}`);
  const all = j.values || [];
  const map = {};
...
// id 기준 upsert. rating/note 중 전달된 것만 갱신.
export async function upsertReview(id, rating, note) {
  await ensureReviewTab();
  const j = await api(`/values/${enc(q(REVIEW_TAB))}!A:A`);
  const col = j.values || [];
  let rowNum = -1;
  for (let r = 1; r < col.length; r++) {
    const v = String((col[r] && col[r][0]) || '').replace(/^'/, '');
  ...
  const cur = await api(`/values/${enc(q(REVIEW_TAB))}!A${rowNum}:D${rowNum}`);
  const row = (cur.values && cur.values[0]) || [String(id), '', '', ''];
  const newRating = rating !== undefined ? rating : (row[1] || '');
  const newNote = note !== undefined ? note : (row[2] || '');
