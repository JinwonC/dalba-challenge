...
export default function Home() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(j.videos || []); setCount(j.count || 0);
  ...
  const saveReview = async (id, patch) => {
    ...
    try {
      ...
      setTimeout(() => setRows((prev) => prev.map((v) => (v.id === id ? { ...v, _saved: false } : v))), 1200);
    } catch (e) {
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', padding: 20, maxWidth: 1400, margin: '0 auto', color: '#111' }}>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginBottom: 14 }}>
        ...
        <button onClick={load} disabled={loading} style={btn}>{loading ? '불러오는 중…' : '조회'}</button>
        <span style={{ color: '#888', fontSize: 13 }}>{count.toLocaleString()}건 (상위 {rows.length} 표시)</span>
      </div>
              ...
              <tr key={v.id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={td}>
                  <div style={{ display: 'flex', gap: 3 }}>
                    {RATINGS.map((rt) => (
                      <button key={rt} onClick={() => saveReview(v.id, { rating: v.rating === rt ? '' : rt })}
                ...
                <td style={td}>
                  <NoteCell value={v.note} saving={v._saving} saved={v._saved}
                    onSave={(note) => saveReview(v.id, { note })} />
                </td>
              </tr>
            ))}
            {!rows.length && !loading && (
