...
| `d'Alba_Pickdi_Process` | duplicate | `duplicate` 탭 | 리스팅 |
...
**duplicate 모드**는 `duplicate` 탭 하나만 읽는다. 이 탭은 제품 탭 7개를 `VSTACK`으로 쌓은
통합 뷰이고 시트의 조건부서식도 같은 탭을 참조하므로, **웹 페이지와 시트가 정확히 같은 데이터를 본다.**
헤더 행이 없어 열 번호로 읽는다 — `A 제품탭 / B 담당자 / C 리스팅일 / D 핸들 / L O·X / M Reason / N 1st Email Sent`.
제품 탭 원본은 읽지 않는다. duplicate가 곧 그것들의 합이라 두 번 세게 된다.
열 위치가 밀리면 조용히 망가지므로, 담당자(B열)가 절반 미만으로만 채워지면 빌드 로그에 경고가 뜬다.
...
중복이 잡히면 판정 아래에 구조가 한 줄로 나온다.

```
담당자 2명 · 제품 탭 2곳 · 2026-07-20 → 2026-07-27
```

담당자 몇 명이 제품 탭 몇 곳에 걸쳐, 언제부터 언제까지 리스팅했는지다. 그 아래에는 건별로
**누가 · 어느 시트/제품 · 리스팅일 · 컨택 여부 · O·X 판정**이 붙는다. 위 예시라면 Hannie가 7/20에
X 판정을 냈는데 Jane이 7/27에 다시 리스팅하고 7/29에 DM까지 보냈다는 뜻이라, 왜 중복이
생겼는지가 바로 읽힌다.
...
- **블랙리스트 탭은 읽지 않는다.** 지정된 5개 탭만 보도록 범위를 정했기 때문이다.
  ...
  판정 등급을 하나 늘리면 된다.
- 구 시트 `(new)` 탭은 **담당자별**로 나뉘어 있고 탭 이름이 곧 담당자다. 담당자 컬럼이 없으므로
  탭 이름에서 `(new)` 접두사를 떼어 담당자로 쓴다. 신 시트는 `VN Owner`, 유가 시트는 `담당자`
  컬럼이 있어 그대로 쓴다.
...
워크플로 로그의 마지막 요약에서 `다른 담당자`와 `본인`을 구분한다.

```
인하우스인데 다시 리스팅됨 — 다른 담당자 0명 / 본인 97명(정상 퍼널)
유가 협업 중인데 다시 리스팅됨 — 다른 담당자 76명 / 본인 0명(정상 퍼널)
```

**본인**은 자기가 발굴해서 자기가 전환시킨 경우라 낭비가 아니다 — 정상적인 퍼널이다.
문제는 **다른 담당자** 쪽이다. 남이 이미 잡아둔 사람을 또 리스팅했다는 뜻이고,
그중에서도 "유가 협업 중"과 "제안 발송"은 실제 사고로 이어질 수 있는 숫자다.
